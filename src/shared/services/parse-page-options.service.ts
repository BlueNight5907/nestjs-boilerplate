...
import { InvalidPageOptionsException } from 'exceptions/page';
import {
  type IFilterOptions,
  type IFilterQuery,
  type ISearchOptions,
...
import { type IParsePageOptionsService } from 'shared/interfaces';
...
export class ParsePageOptionsService implements IParsePageOptionsService {
  parseSearchOptions(options: ISearchQuery): ISearchOptions {
    ...
    if (isNil(search) || isNil(searchBy)) {
      throw new InvalidPageOptionsException(`search=${search ?? searchBy}`);
    }
